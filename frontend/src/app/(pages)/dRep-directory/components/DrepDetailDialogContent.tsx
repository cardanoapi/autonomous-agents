...
export default function DrepDetailDialogContent({
    ...
    const formattedVotingPower = convertLovelaceToAda(dRep.votingPower).toLocaleString('en-US');

    const redirectToGovTool = () => {
        window.open(`https://govtool.cardanoapi.io/connected/drep_directory/${dRep.drepId}`, '_blank');
    };

    return (
        <AppDialogContent className=" pt-0" onClose={onClose}>
            <div className="mb-4 flex items-center gap-2">
                <span className="text-lg font-semibold">{getDrepGivedName(dRep) || 'Data Missing'}</span>
                <ExternalLink className="cursor-pointer text-blue-600" onClick={redirectToGovTool} />
            </div>
            <div className="w-full space-y-4 rounded-lg border border-gray-200 p-4 shadow-sm md:w-[520px]">
                ...
                <Typography className="text-sm text-gray-800">Voting Power: {formattedVotingPower} ADA</Typography>

                {dRep.references && dRep.references.length > 0 && (
                    <Typography className="text-sm text-gray-800">References: {dRep.references.join(', ')}</Typography>
                )}

                <Typography className="text-sm text-gray-700">{dRep.bio || 'No bio available'}</Typography>

                <DRepDetails dRep={dRep} />

                {dRep.agentId && dRep.agentName && (
                    <div className="space-y-1">
                        <Typography className="text-sm text-gray-800">Agent ID: {dRep.agentId}</Typography>
                        <Typography className="text-sm text-gray-800">Agent Name: {dRep.agentName}</Typography>
                    </div>
                )}
                {!dRep.givenName && (
                    <Typography className="text-sm text-red-600">
                        The data used when this DRep was created has been formatted incorrectly.
                    </Typography>
                )}
            </div>
        </AppDialogContent>
...
const DRepDetails = ({ dRep }: { dRep: IDRepInternal }) => {
    ...
    const renderExternalUrl = () => (
        <Typography className="flex items-center gap-2 text-sm text-gray-700 ">
            External URL:
            {dRep.url ? (
                <Typography onClick={openExternalUrl} className=" cursor-pointer text-blue-800">
        ...
        <p className="flex items-center gap-2 text-sm text-gray-700">
            ...
            {dRep.latestTxHash ? (
                <div className="flex w-80 items-center">
                    ...
                    <CopyIcon onClick={() => copyToClipboard(dRep.latestTxHash || '', 'Tx Hash copied')} />
                </div>
            ) : (
        ...
        <Typography className="flex items-center gap-2 text-sm text-gray-700">
            ...
            {dRep.metadataHash ? (
                <div className="flex w-64 items-center">
                    ...
                    <CopyIcon onClick={() => copyToClipboard(dRep.metadataHash || '', 'Metadata Hash copied')} />
                </div>
            ) : (
    ...
    const renderLatestRegistrationDate = () => (
        <Typography className="flex items-center gap-2 text-sm text-gray-700">
            ...
            {dRep.latestRegistrationDate ? (
                <Typography className="truncate">{new Date(dRep.latestRegistrationDate).toLocaleString()}</Typography>
            ) : (
