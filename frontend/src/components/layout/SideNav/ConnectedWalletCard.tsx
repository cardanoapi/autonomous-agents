...
export default function ConnectedWalletCard({ className, onDisconnect }: { className?: string; onDisconnect?: any }) {
    const [currentConnectedWallet] = useAtom(currentConnectedWalletAtom);

    return (
        <div className={cn('flex flex-col gap-1 rounded-lg border-2 border-brand-Gray-400 p-2', className)}>
