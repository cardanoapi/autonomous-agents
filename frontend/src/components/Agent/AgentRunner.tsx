...
const AgentRunnerComponent = ({ agent }: { agent?: IAgent }) => {
    const dockerCommand = `docker run -d --pull always -e WS_URL=${environments.NEXT_PUBLIC_WS_URL} -e AGENT_ID=${agent?.id} cardanoapi/autonomous-agents-agent-node:${environments.NEXT_PUBLIC_IMAGE_TAG}`;
    return (
