        ...
        <>
            {agent?.is_active && (
                <>
                    ...
                    <div className={'flex h-full w-full flex-col gap-12 overflow-y-auto px-2'}>
                        {AvailableFunctions.map((func) => {
                            return (
                                <div key={func.group} className="flex flex-col gap-4">
                                    <span className={'inline-flex font-semibold text-brand-Black-300'}>
                                        {func.group}
                                    </span>
                                    <div className={'flex flex-wrap gap-x-6 gap-y-4 px-2 '}>
                                        {func.items.map((item) => {
                                            return (
                                                <div
                                                    onClick={() => handleOnClickFunctionsTab(item)}
                                                    key={item.id}
                                                    className={
            ...
            <Dialog open={openDialog} onOpenChange={handleOpenChange}>
                <DialogContent
                    className={'overflow-hidden !p-0 max-md:w-full max-sm:h-full md:!min-w-[650px]'}
                    onClickCloseIcon={() => handleOnClickCloseIcon()}
                >
                    <FormRenderer agent={agent} closeModal={(boolVal) => handleOpenChange(boolVal)} />
                </DialogContent>
            </Dialog>
