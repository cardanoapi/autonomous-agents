...
const DropdownMenuTrigger = ({
    ...
    useAddIcon?: boolean;
    disableIcon?: boolean;
    renderChevronIcon?: boolean;
}) => (
    <DropdownMenuPrimitive.Trigger
        {...props}
        className={cn(
            'flex items-center gap-x-2 focus:outline-none',
            'group',
            { 'rounded-[4px] border-[1px] border-brand-border-300 px-4 py-1': border },
            className
        )}
    >
        {children}
...
>(({ className, inset, children, ...props }, ref) => (
    <DropdownMenuPrimitive.SubTrigger
        ref={ref}
        className={cn(
            'flex cursor-default select-none items-center rounded-sm border-2 px-2 py-1.5 text-sm focus:bg-white data-[state=open]:bg-white',
            inset && 'pl-8',
            className
        )}
        {...props}
