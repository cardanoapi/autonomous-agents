...
class TemplateTriggerService:
    ...
    async def update_configurations_for_template(
        ...
        updated_configs = await self.update_template_multiple_configs(template_configurations)
        await self.delete_template_multiple_configs(configs_to_delete)
        return updated_configs

    async def update_template_multiple_configs(self, template_configurations: List[TemplateTriggerResponse]):
        new_configs = [config for config in template_configurations if config.id == ""]
        updating_configs = [config for config in template_configurations if config.id != ""]
        updated_configs = []
