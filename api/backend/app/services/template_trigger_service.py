...
class TemplateTriggerService:
    ...
    async def update_configurations_for_template(
        ...
        updated_configs = await self.update_template_multiple_configs(
            template_configurations, template_existing_config_ids
        )
        await self.delete_template_multiple_configs(configs_to_delete)
        return updated_configs

    async def update_template_multiple_configs(
        self, template_configurations: List[TemplateTriggerResponse], template_existing_config_ids
    ):
        new_configs = [
            config
            for config in template_configurations
            if config.id == "" or config.id not in template_existing_config_ids
        ]
        updating_configs = [
            config
            for config in template_configurations
            if config.id != "" and config.id in template_existing_config_ids
        ]
        updated_configs = []
