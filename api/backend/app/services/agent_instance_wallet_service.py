...
def generate_wallet_key(agent_index: int, instance_index: int) -> dict[str, Any]:
    ...
    payment_path = f"m/1852'/1815'/0'/0/{instance_index}"
    staking_path = f"m/1852'/1815'/0'/2/{instance_index}"
...
class AgentInstanceWalletService:
    ...
    async def create_wallet(self, agent: AgentResponse):
        wallet_details = [
            AgentInstanceWallet(
                **generate_wallet_key(agent.index, instance), agent_id=agent.id, instance_index=instance
            )
    ...
    async def update_wallet(self, agent: AgentResponse, updating_wallet_instance: int):
        wallet_details = [
            AgentInstanceWallet(
                **generate_wallet_key(agent.index, instance + agent.instance),
                agent_id=agent.id,
    ...
    async def get_wallets(self, agent_id: str) -> List[AgentInstanceWallet]:
        ...
        for wallet in wallets:
            wallet.payment_key_hash = (str(wallet.payment_key_hash).encode().hex(),)
            wallet.stake_key_hash = (str(wallet.stake_key_hash).encode().hex(),)
