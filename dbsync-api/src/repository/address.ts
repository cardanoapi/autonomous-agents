...
export const fetchFaucetBalance= async (address:string)=>{
  const result  = await prisma.$queryRaw`
      SELECT COALESCE(SUM(utxo_view.value::numeric), 0) + COALESCE(SUM(reward_rest.amount), 0) + COALESCE(SUM(r.amount), 0) AS total_value
      FROM stake_address
               ...
               LEFT JOIN reward r ON r.addr_id = stake_address.id
