...
export const fetchDrepDetails = async (drepId: string,isScript?:boolean) => {
    ...
    const result = (await prisma.$queryRaw`
WITH dreps AS ( SELECT  id,has_script FROM drep_hash
               WHERE drep_hash.raw = decode(${drepId}, 'hex') and  (has_script = ${scriptPart[0]} or has_script = ${scriptPart[1]})),
               drep as  (SELECT  dreps.* from drep_registration  join dreps  on dreps.id = drep_registration.drep_hash_id   where deposit > 0 order by tx_id desc limit 1),
     ...
     WasRegisteredAsDRep AS (SELECT (EXISTS (SELECT *
                                             FROM drep_registration
                                                      JOIN drep ON drep.id = drep_registration.drep_hash_id
                                             WHERE
                                                drep_registration.voting_anchor_id IS NOT NULL)) AS value),
     WasRegisteredAsSoleVoter AS (SELECT (EXISTS (SELECT *
                                                  FROM drep_registration
                                                           JOIN drep ON drep.id = drep_registration.drep_hash_id
                                                  WHERE
                                                     drep_registration.voting_anchor_id IS NULL)) AS value),
     ...
     CurrentVotingPower AS (SELECT amount AS amount
                            FROM drep
                                     LEFT JOIN drep_distr ON drep_distr.hash_id = drep.id
     ...
     SoleVoterRetire AS (SELECT encode(AllRegistrationEntries.tx_hash, 'hex') as tx_hash
                         ...
                         ORDER BY AllRegistrationEntries.tx_id asc
                         LIMIT 1),
     IsScriptHash AS (SELECT EXISTS (SELECT drep.has_script
                                     FROM drep
                                     WHERE drep.has_script = true) AS has_script),
